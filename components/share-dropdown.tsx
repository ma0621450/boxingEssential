...
export function ShareDropdown({ url, title, image, className, align = "left" }: ShareDropdownProps) {
  ...
  return (
    <div ref={containerRef} className={cn("relative", className)}>
      ...
      {open && (
        <div
          className={cn(
            "absolute top-full mt-2 z-50 flex items-center gap-1.5 p-2 rounded-xl border border-border/50 bg-card shadow-lg",
        ...
        >
          {shareLinks.map((link) => (
            <a
              ...
              className={cn(
                "w-9 h-9 flex items-center justify-center rounded-lg bg-secondary text-secondary-foreground transition-all duration-200",
                link.color
              )}
