...
export function TrainingCategoryClient({
  ...
  const categoryDetails = {
    gym: {
      ...
      description:
        "Build the foundational strength, explosive power, and physical conditioning of a professional fighter. Access high-quality strength circuits, weight room walkthroughs, and core stability programs designed to unlock your body's athletic peak.",
    ...
    boxing: {
      ...
      description:
        "Master the sweet science under the guidance of elite coaches. Access structured video drills detailing perfect punching mechanics, lateral footwork, pocket defense, head movement, and inside sparring combinations.",
    ...
    fitness: {
      ...
      description:
        "Get in fighter shape without ever taking a punch. Our boxing fitness routines blend high-volume shadowboxing combinations, high-intensity interval conditioning (HIIT), and agility ladder flows to burn calories and build stamina.",
        ...
        {!featuredVideo ? (
          ...
          <div className="bg-zinc-950 dark:bg-zinc-900 border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl">
            <div className="grid grid-cols-1 lg:grid-cols-12">
              <Link
                href={toSitePath(`/videos/${featuredVideo.slug}`)}
              ...
              >
                ...
                <div className="absolute top-4 left-4 bg-red-600 text-white text-[10px] md:text-xs font-black uppercase tracking-widest px-3 py-1.5 rounded-full flex items-center gap-1.5">
                  <CategoryIcon className="h-3.5 w-3.5" /> {featuredVideo.lessonLabel}
                </div>
              ...
              <div className="lg:col-span-5 p-8 md:p-10 flex flex-col justify-center text-white">
                ...
                <div className="border-t border-zinc-800 pt-6">
                  <span className="text-xs text-zinc-500 uppercase block font-semibold">
                    Featured {featuredVideo.lessonLabel}
                  </span>
      ...
      <section className="mb-16">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-border/60 pb-5 mb-8 gap-4">
          <div>
            <h2 className="text-2xl md:text-3xl font-black tracking-tight text-foreground flex items-center gap-2">
              <CategoryIcon className={cn("h-6 w-6", categoryDetails.themeColor)} /> Video
              Library
            </h2>
            <p className="text-muted-foreground text-sm mt-1">
              {categoryVideos.length} lessons · ordered Lesson 1 through Lesson{" "}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {paginatedVideos.map((vid) => (
            <Link
              key={vid.slug}
              href={toSitePath(`/videos/${vid.slug}`)}
            ...
            >
              <div className="relative aspect-video bg-black shrink-0">
                ...
                <div className="absolute top-2 left-2 bg-red-600 text-white text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full">
                  {vid.lessonLabel}
                </div>
        ...
        {totalPages > 1 && (
          <div className="mt-12 flex items-center justify-center gap-2">
            <button
              ...
              className="p-2 border border-border rounded-xl hover:bg-secondary transition disabled:opacity-40"
